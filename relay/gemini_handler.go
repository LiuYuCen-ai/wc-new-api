...
func GeminiImageHelper(c *gin.Context, info *relaycommon.RelayInfo) (newAPIError *types.NewAPIError) {
	info.InitChannelMeta(c)
	...
	// 确保设置 responseModalities 为图片生成
	if request.GenerationConfig.ResponseModalities == nil || len(request.GenerationConfig.ResponseModalities) == 0 {
	...
	logger.LogDebug(c, "Gemini image request body: %s", jsonData)
	...
	resp, err := adaptor.DoRequest(c, info, requestBody)
	if err != nil {
		logger.LogError(c, "Do gemini image request failed: "+err.Error())
		return types.NewOpenAIError(err, types.ErrorCodeDoRequestFailed, http.StatusInternalServerError)
	}
